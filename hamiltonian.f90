...
implicit none 
    ...
    ! subroutine generate_hamiltonian(thrds, ti, unit, prms, sites, nbb, trisites, dim, basis, hamOff, nOff, k1, k2, tilted, nHel, tilt, lx, ly, ucx, ucy, orbsize, norm, norm2D, orbits, phases, xtransl, ytransl, symmetrize, id, par, rot, refl, c6, t, rcoff, rcdi, parities, dplcts, hamOff_dp, hamDi_dp, nDi_off, hamOff_dc, hamDi_dc, hamDi_c2, nnnbb, hexsites, hamDi, occ, nDi)
        ...
        ! character(len=*), intent(inout)              :: param_list
        ...
        call par2(unit, k1, k2, -1, 0.d0, 0.d0, param_list, type)
        call loadham(type, param_list, unit, ti, sites, nOff, nDi, hamDi, hamOff, rcoff, hamOff_dp, hamOff_dc, occ, exist1, exist2, exist3) ! Loads the sub-Hamiltonians from file if they exists. If not, sets corresponDing exist1, exist2, exist3 to .false. and generates the Hamiltonian.
        ...
        if(.not.(exist1)) then ! Generates the off-diagonal part of the Hamiltonian if it does not exist yet.
            if(ti == 0) then ! If no translation symmetry is used, generate the Hamiltonian with the standard hopping procedure
                ! call hopping(flag_i)
                call hopping_i(othrds, unit, param_list, sites, nnBonds, bsites, dim, basis_states, hamOff, nOff)
    ...
    !With variable arguments
    subroutine hopping_i(threads, unit, parameters, sites, nbonds, bsites, dim, basis_list, ham, nnz)
    ...
    subroutine hopping_dc(unit, params, gencluster, nHel, tilt, lx, ly, ucx, ucy, sites, nbonds, dim, basis, bsites, norm, xtransl, ytransl, t, k1, k2, rc, ham, nnz)
    ...
    subroutine diagonal_dp(unit, parameters, sites, nbonds, nbonds2, dim, bsites, bsites2, basis, ham, occ, nnz)
